<div class="dropdown topbar-head-dropdown ms-1 header-item" id="notificationDropdown">
    <button type="button" class="btn btn-icon btn-topbar btn-ghost-secondary rounded-circle"
        ...
        <span class="position-absolute topbar-badge fs-10 translate-middle badge rounded-pill bg-danger"
              id="unreadCount">
            {{ \App\Models\tbl_notifications::where('fk_user_id', Session::get('user_id'))
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-auto">
                        <span class="badge bg-light-subtle text-body fs-13" id="totalNotifications">
                            {{ \App\Models\tbl_notifications::where('fk_user_id', Session::get('user_id'))->where('is_read', false)->where('is_removed', false)->count() }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="tab-pane fade show active py-2 ps-2" id="all-noti-tab" role="tabpanel">
                <div data-simplebar style="max-height: 300px;" class="pe-2">
                    <ul id="notificationsList" class="list-unstyled">
                        ...
                        @foreach (\App\Models\tbl_notifications::where('fk_user_id', Session::get('user_id'))->where('is_removed', false)->orderBy('is_read', 'asc')->orderBy('create_datetime','desc')->get() as $notification)
                            <li class="text-reset notification-item d-block dropdown-item" onclick="markAsRead({{ $notification->id }})">
                                <div class="d-flex">
                                    ...
                                    <div class="flex-grow-1">
                                        <a href="{{ $notification->redirect_url ?? '#!' }}" class="stretched-link">
                                            <h6 class="mt-0 mb-2 lh-base">{{ json_decode($notification->data)->message }}</h6>
                                            <p class="mb-0 fs-11 fw-medium text-uppercase text-muted">
                                                <span><i class="mdi mdi-clock-outline"></i> {{ \Carbon\Carbon::parse($notification->create_datetime)->diffForHumans() }}</span>
                                            </p>
                                        </a>
                                    </div>
                                </div>
                            ...
                            <div class="d-flex align-items-center ms-2">
                                ...
                                <button class="btn btn-sm btn-outline-danger"
                                        onclick="removeNotification({{ $notification->id }})">Remove
                                </button>
                            </div>
                        @endforeach
                    </ul>
...
<script>
    function markAsRead(notificationId) {
        const unreadCountElement = document.getElementById('unreadCount');
        const totalNotificationsElement = document.getElementById('totalNotifications');
        const url = `{{ route('notifications.markAsRead', ['id' => '__ID__']) }}`.replace('__ID__', notificationId); // Replace placeholder with actual ID
        $.ajax({
            url: url,
            type: 'POST',
            headers: {
    ...
    function removeNotification(notificationId) {
        const unreadCountElement = document.getElementById('unreadCount');
        const totalNotificationsElement = document.getElementById('totalNotifications');
        const url = `{{ route('notifications.remove', ['id' => '__ID__']) }}`.replace('__ID__', notificationId); // Replace placeholder with actual ID
        $.ajax({
            url: url,
            type: 'POST',
            headers: {
