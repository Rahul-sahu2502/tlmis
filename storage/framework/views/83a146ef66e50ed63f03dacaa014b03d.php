<script>
    $(jQuery).ready(function() {
        ...
        let send_back = true;
        let session_user_id = <?php echo e(session('user_id')); ?>;
        ...
        function task_reply_list(task_id) {
            // console.log(task_id);
            $.ajax({
                url: "<?php echo e(url('task-reply-list')); ?>/" + task_id, // Route to the controller
                type: "get",
                ...
                success: function(response) {
                    ...
                    if (response.length) {
                        ...
                        $.each(response, function(key, value) {
                            ...
                            if(<?php echo e(session('user_id')); ?>==value['created_by']){
                                $("#send_back").hide();
                            }
                            let file = "";
                            if (value['reply_file'] != null) {
                                let file_path=`"<?php echo e(asset('uploads')); ?>/${value['reply_file']}"`;
                                if("<?php echo e(config('app.env')); ?>" =="production"){
                                    file_path =  `"<?php echo e(config('custom.file_point')); ?>${value['reply_file']}"`;
                                }
        ...
        let task_id = <?php echo e($details[0]->task_id); ?>;
        ...
        $('#reply_form').on('submit', function(e) {
            ...
            $("#reply_msg").html("");
            $.ajax({
                url: "<?php echo e(route('task_reply')); ?>", // Route to the controller
                method: "POST",
                data: formData,
            ...
            if (reply_confrim) {
                ...
                SwalWithCustomSettings.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "<?php echo e(route('task_close')); ?>", // Route to the controller
                            type: "post",
                            headers: {
        ...
        document.getElementById("send_back") && document.getElementById("send_back").addEventListener("click", function() {
            if (send_back) {
                ...
                SwalWithCustomSettings.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "<?php echo e(route('task_send_back')); ?>", // Route to the controller
                            type: "post",
                            headers: {
                                'X-CSRF-TOKEN': "<?php echo e(csrf_token()); ?>"
                            },
        ...
        $('#docs_form').on('submit', function(e) {
            ...
            $(':input[type="submit"]').prop('disabled', true);
            $.ajax({
                url: "<?php echo e(route('attach_supporting_docs')); ?>", // Route to the controller
                method: "POST",
                data: formData,
                processData: false, // Don't process the files
                contentType: false, // Don't set content type
...
<?php /**PATH C:\xampp\htdocs\laravel_projects\TLMIS original code\TLMIS\resources\views/tasks/overview_js.blade.php ENDPATH**/ ?>